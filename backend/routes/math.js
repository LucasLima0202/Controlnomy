      ...
      SELECT 
        FORMAT(
          ...
          (SELECT COALESCE(SUM(amount), 0) FROM transactions 
            WHERE type = 0 AND user_id = 1 AND MONTH(date) = ? AND YEAR(date) = ?) - 
          (SELECT COALESCE(SUM(amount), 0) FROM transactions 
            WHERE type = 1 AND user_id = 1 AND MONTH(date) = ? AND YEAR(date) = ?), 2
        ) AS saldo_atual
      FROM users WHERE id_user = 1`;

    db.query(sql, [month, year, month, year], (err, result) => {
        if (err) {
